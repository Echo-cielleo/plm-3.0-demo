...
with sync_playwright() as pw:
    ...
    ok('新增物质' in df and '字段修改' in df and '废止物质' in df, '版本比较覆盖真实结构化草稿的新增/字段修改/废止')
    ok('specificLimits' in df and 'classifications' in df, '版本比较逐字段显示 SCL 与分类变化')
    ok('演示结构化草稿实时比较' in df, '预览明确说明与当前数据集实时比较')
    ok('未解析真实 XLSX' in df, '明确演示导入边界')
    ...
    ok(page.evaluate("()=>clpViDatasetGet('CLP-VI-ATP23').effectiveFrom") == '2027-02-01' and page.evaluate('()=>CLP_MODULES.vi.ver') == 'ATP 22',
       '发布待生效 ATP 23 快照，当前模块仍为 ATP 22')
    ok(page.evaluate("()=>clpViDatasetGet('CLP-VI-ATP23').cutoff") == '2026-09-10', '新快照记录数据截止日期')
    ok(page.evaluate('()=>CLP_VI_ROWS.length') == n_vi and page.evaluate("()=>clpViRecords('2027-02-01').length") == n_vi + 1,
       '未来版本新增 1 条，当前兼容投影不提前切换')
    ok(page.evaluate("()=>clpViRowsProjection('2027-02-01').some(r=>r.name==='2-乙基己酸锆'&&r.ver==='ATP 23')"), '新增条目归属 ATP 23 快照')
    ok(page.evaluate("()=>clpViRowsProjection('2027-02-01').every(r=>r.ver==='ATP 23')"), '未来快照 6 条记录统一归属 ATP 23')
    ...
    page.wait_for_timeout(220)
    ok(page.locator('#clpTable tbody tr').count() == 5, 'Tab1 默认仍展示当前生效的 5 条')
    vs = page.evaluate("()=>[].map.call($('clpViVer').options,x=>x.text)")
    ok('ATP 23' in vs and 'ATP 22' in vs, '版本下拉同时保留历史和待生效快照：%s' % ' / '.join(vs))
    page.select_option('#clpViVer', 'ATP 23'); page.wait_for_timeout(120)
    ok(page.locator('#clpTable tbody tr').count() == 6 and 'ATP 23' in page.locator('.clp-strip').first.inner_text() and '2027-02-01' in page.locator('.clp-strip').first.inner_text(),
       '选择 ATP 23 后表格与信息条同步展示未来快照')
    page.evaluate("()=>{_clpF.rules={kw:'',tgt:'',st:'',eng:''};clpLGoTab('rules');}")
    page.wait_for_timeout(220)
