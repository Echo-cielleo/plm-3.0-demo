...
(function(){
  ...
  var extra={Q004:{reviewDue:'2027-02-02',latestVersion:'2026-Q1'}};
  LAW_QUERY_REFERENCE_ROWS.forEach(function(r){
...
function lawConfirmReview(id){
  ...
  LAW_QUERY_REFERENCE_ROWS.filter(function(x){return x.lawKey===r.key;}).forEach(function(x){x.reviewDue=r.reviewDue;x.status='已生效';});
...
function lawFinish(){
  ...
  if(_lawCur){_lawCur.ver=_lawTmp.ver;_lawCur.latestVer=_lawTmp.ver;_lawCur.eff=_lawTmp.eff;_lawCur.upd=nowStr();_lawCur.lastReview=todayStr();_lawCur.reviewDue=daysFromNow(365);_lawCur.status='已生效';_lawCur.verifier=v;_lawCur.items+=12;LAW_QUERY_REFERENCE_ROWS.filter(function(x){return x.lawKey===_lawCur.key;}).forEach(function(x){x.version=_lawCur.ver;x.latestVersion=_lawCur.ver;x.reviewDue=_lawCur.reviewDue;x.status='已生效';});}
